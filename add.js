...
const addItemToCart = (name, price, imageSrc) => {
  const cartItemsList = document.querySelectorAll(".cart-item");
  let existingItem = Array.from(cartItemsList).find(item => item.dataset.name === name);
  ...
  if (existingItem) {
    ...
    existingItem.querySelector('.total-price').textContent = `$${(newQuantity * price).toFixed(2)}`;
    updateTotalCartQty();
  } else {
    // Add new item
